...
```{r}
d.diagnosticity <- read_csv(here('data/diagnositicity/graphical_conventions_group_data_run5_submitButton_diagnosticity.csv')) %>%
...
d.diagnosticity %>%
  ...
  lmer(scale(diagnosticity) ~ phase * condition + (1 + phase + condition | target) + (1 +phase +  condition | gameID),
       data = .,
...
```{r}
d.overlap <- read_csv(here('data/diagnositicity/graphical_conventions_group_data_run5_submitButton_sketchOverlap.csv')) %>%
  group_by(obj_id) %>%
